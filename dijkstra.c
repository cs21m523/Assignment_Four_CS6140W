...
void dijkstra(unsigned int **graph, int vertices, int src, int dst)
{
    int i = 0, j = 0, k = 0;
    ...
    for (i = 0; i < vertices; i++)
    {
        ...
        for (k = 0; k < vertices; k++)
        {
            ...
            if (!graph[minindex][k])
                continue;
            if (distance[minindex] == INFINITY)
                continue;
            if (distance[minindex] + graph[minindex][k] < distance[k])
            {
...
void main(int argc, char *argv[])
{
    ...
    if (choice != 'y' && choice != 'Y')
    {
        ...
        getchar();
        printf("Enter start node from %c to %c> ", 'A', 'A' + vertices);
        scanf("%c", &choice);
        ...
        printf("Enter end node from %c to %c> ", 'A', 'A' + vertices);
        fflush(stdout);
        scanf(" %c", &choice);
        if (choice >= 'A' && choice <= 'Z')
        {
    ...
    dijkstra(graph, vertices, src, dst);
}